function Pipeline_surface_ips
% Run the surface reconstruction for each participant (tested in person, 
...
% load images used as inputs for surface reconstruction
load([workDir 'ims_unfam_surface.mat']) % this image cell array is generated by ResizeImages.m
imgFile = ims_unfam_surface; % 57 x 1 cell array (each cell is a surface image)
cd(workDir);
...
saveOut = 0;
% saveFileDir_bck = workDir;
[outMask,bck_common] = commonbck(imgFile);

% Save the background index and outMask
if saveOut==1
    cd(workDir)
    save('bck_common.mat','bck_common')
    save('outMask.mat','outMask')
end

% Note: bck_common.mat and outMask.mat are the common background files for
% surface of the 57 stimuil. The naming was a bit confusing so I duplicated
% files that were named as bck_common_srf.mat and outMask_srf.mat. 

%% Applying the background mask on all images 
run = 0;
if run==1
    ims_unfam_surface_masked = applyingMask(imgFile,outMask);
    save('ims_unfam_surface_masked.mat','ims_unfam_surface_masked');
else
    load('ims_unfam_surface_masked.mat')
end
imgFileMasked = ims_unfam_surface_masked;

%% Surface Reconstruction
for s = 1:length(subs_all)
    subID = subs_all{s}; % subject ID
    saveFileDir = [rootDir 'analysis/recon_surface_ips/' group subID '/']; % where to save the reconstruction output
    if ~exist(saveFileDir,'dir')
        mkdir(saveFileDir);
    end
    
    % check whether perception-based surface reconstruction has been conducted
    cd(saveFileDir)
    fileList = dir(['final_Tp_srf' group subID '*.mat']);
    ...
    if recon_percep==0 % reconstruction for this subject has not been conducted
        ...
        % Surface reconstruction
        disp(['subject ' group subID ' surface reconstruction started'])
        clear data_reconSrf_Tp
        data_reconSrf_Tp = reconSrf(group,subID,parameter,saveFileDir,...
            workDir,conf,imgFileMasked,bck_common,objAccPerm);        
        ...
        outFile = [saveFileDir 'final_Tp_srf_' group num2str(subID) '_' date];
        if superSub
            save(outFile, 'data_reconSrf_Tp','superSub','allSubs');
        else
            save(outFile, 'data_reconSrf_Tp');
        end
        disp(['subject ' group subID ' surface reconstruction done'])
        
    else % reconstruction for this subject has been done
        disp(['subject ' group subID ' surface reconstruction started'])
        disp(['subject ' group subID ' surface reconstruction has been conducted'])
    end
